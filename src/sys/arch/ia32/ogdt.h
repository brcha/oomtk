#ifndef __IA32_OGDT_H__
#define __IA32_OGDT_H__
/*
...
class OGDT
{
  ...
  protected:
     ...
     */
    inline void setupLimitBase(size_t cpu, word_t seg, uint32_t limit, uint32_t base)
    {
      // Setup the limit
      m_gdt[cpu][seg].bits.limit_lo  = limit & 0x0000ffff;
      m_gdt[cpu][seg].bits.limit_hi  = (limit & 0x000f0000) >> 16;
      // Setup the base
      m_gdt[cpu][seg].bits.base_lo   = base & 0x00ffffff;
      m_gdt[cpu][seg].bits.base_hi   = (base & 0xff000000) >> 24;
    };
  ...
  protected:
	...
	uint64_t  base_lo     : 16; ///< @brief Base address (low)
        uint64_t  base_mid    : 8;  ///< @brief Base address (mid)
    ...
    /// @brief Global descriptor table
    SegmentDescriptor_t m_gdt[MAX_NCPU][seg_Count];
};
